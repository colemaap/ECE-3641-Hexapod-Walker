...
#include <Servo.h>
#include "SR04.h"
#include <Leg.h> //Unused
#define NEUTRAL_ANGLE 90
#define OFFSET 45
#define STEP_ANGLE 30
#define FOOT_LIFT 30
#define STEP_DELAY 20 //20 seems to work well
#define DRAG_DELAY 7
#define OBST_DIST 20


//The robot kind of freaks out when I use servos in another class, so I have to use functions instead
...
{
    static bool stage = 0; //Determines which triplet of legs is doing what. Allows this function to represent one step instead of 2.
    bool done = false;
    if (stage == 0)
    {
    legJoints[0].write(NEUTRAL_ANGLE - lift_angle); //Lift pair 1's feet
    ...
    legJoints[1].write(NEUTRAL_ANGLE + left_step_angle - OFFSET); //swivel pair 1's hips forward
    legJoints[9].write(NEUTRAL_ANGLE - right_step_angle);
    legJoints[5].write(NEUTRAL_ANGLE + left_step_angle + OFFSET);
    delay(STEP_DELAY);
    
    legJoints[0].write(NEUTRAL_ANGLE); //Drop pair 1's feet
    ...
    {
    legJoints[6].write(NEUTRAL_ANGLE + lift_angle); //Lift pair 2's feet
    ...
    legJoints[7].write(NEUTRAL_ANGLE - right_step_angle + OFFSET); //swivel pair 2's hips forward
